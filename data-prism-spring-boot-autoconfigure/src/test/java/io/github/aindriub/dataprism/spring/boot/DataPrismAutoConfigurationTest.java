...
class DataPrismAutoConfigurationTest {
    ...
    /**
     * A configured path this process cannot actually open — here, one whose
     * parent directory does not exist — refuses at startup with {@code
     * AUDIT_SINK_FILE_UNUSABLE} rather than surfacing on the first audited
     * request, and deliberately never repeats the configured path in the
     * refusal message printed to the operator's console at startup, the
     * same choice {@link
     * #refuses_hash_chained_sink_without_a_configured_file_path} pins for
     * {@code MISSING_AUDIT_FILE_PATH}.
     */
    @Test void refuses_hash_chained_sink_with_an_unopenable_file_path(@TempDir Path tempDir) {
        Path file = tempDir.resolve("no-such-parent-directory").resolve("audit.log");
        new WebApplicationContextRunner().withConfiguration(AutoConfigurations.of(DataPrismAutoConfiguration.class))
                .withUserConfiguration(ReviewedHttpIntegrationsWithoutAudit.class).withPropertyValues(valid())
                .withPropertyValues("dataprism.audit.sink=hash-chained", "dataprism.audit.file-path=" + file)
                .run(result -> {
                    ...
                    Throwable failure = rootCause(result.getStartupFailure());
                    assertThat(failure).isInstanceOf(DataPrismConfigurationException.class);
                    assertThat(failure.getMessage()).startsWith("AUDIT_SINK_FILE_UNUSABLE:")
                            .doesNotContain(file.toString())
                            .doesNotContain(file.getFileName().toString());
                });
    }
    /**
